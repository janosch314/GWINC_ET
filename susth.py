...
#____Fused Silica thermo-mechanical properties
    alpha=ifo.Suspension.Silica.Alpha
    beta=ifo.Suspension.Silica.dlnEdT
    C=ifo.Suspension.Silica.C
    K=ifo.Suspension.Silica.K
    rho=ifo.Suspension.Silica.Rho
    phi0=ifo.Suspension.Silica.Phi
    ds=ifo.Suspension.Silica.Dissdepth
    E0=ifo.Suspension.Silica.Y
...
# Marionette and RM stages parameter are loaded.
    ...
    rho1=ifo.Suspension.MaragingSteel.Rho
    C1=ifo.Suspension.MaragingSteel.C
    K1=ifo.Suspension.MaragingSteel.K
    alpha1=ifo.Suspension.MaragingSteel.Alpha
    beta1=ifo.Suspension.MaragingSteel.dlnEdT
    phi1=ifo.Suspension.MaragingSteel.Phi
    E1=ifo.Suspension.MaragingSteel.Y
...
# sec. 2
    Kpend=np.zeros([len(w)],dtype = np.cfloat)
...
# sec. 1.1-1.2-1.3
    ...
    Ah = np.zeros([len(f),2,2],dtype = np.cfloat)
    ...
    Xh=np.zeros([2,len(w)],dtype = np.cfloat)
...
# in a martix formalism Ah*Xh=B. The admittance is calculated. VIR-0.15A-09
# sec. 1.1-1.2-1.3
    Xv = np.zeros([2,len(f)],dtype = np.cfloat)
    Av = np.zeros([len(f),2,2],dtype = np.cfloat)
    ...
    for i in range(len(w)):
        Xv[:,i]=np.linalg.solve(Av[i,:,:],B)


    admitt_v = admittance(Xv[1,:])
    Vertical_PSD = FDT(Temp, admittance_h,w)                         # Vertical suspension thermal noise PSD
...
def Optimisedfibres(f,phi,ifopar):
    ...
    rho=ifo.Suspension.Silica.Rho            # FS density [kg/m^3]
